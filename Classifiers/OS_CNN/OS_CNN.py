...
class OS_CNN(nn.Module):
    def __init__(self,layer_parameter_list,n_class,few_shot = True): #设置架构
        ...
        self.net = nn.Sequential(*self.layer_list) #把所有层顺序相连，形成一个OS-Block
    ...
    def forward(self, X): #train时调用
        
        X = self.net(X) #OS-BLock获得低维度特征

        #######在这里(?)通过OS-Block得到低维度特征后，进行特征的拼接，然后池化？然后全连接得到分类结果？
